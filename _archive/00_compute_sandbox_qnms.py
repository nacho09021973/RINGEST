...
Calcula numericamente los modos quasinormales (QNMs) de cada geometria
sandbox usando el **metodo de shooting**.

Para cada HDF5 en sandbox_v1/01_generate_sandbox_geometries/:
  1. Carga A(z), f(z), z_h, d desde bulk_truth
  2. Plantea la ecuacion de perturbacion escalar:
       _z[e^{(d+1)A} f '] + 2 e^{(d-1)A}/f  = 0
  3. Impone condicion entrante en el horizonte:
        ~ (z_h - z)^{-i/|f'_h|}
  4. Integra numericamente hacia el boundary (z  0)
  5. Busca  complejo tal que (z_min)  0 (condicion QNM)

Guarda los QNMs en:
  - runs/sandbox_v1/qnm_numerical.json  (resumen de todos)
  - Grupo 'qnm_numerical' en cada HDF5 (_real, _imag por modo n)
...
# 
# Geometria: |f'(z_h)| analitico por familia
# 
...
def fp_horizon_analytic(
    ...
    """
    |f'(z_h)| analitico para cada familia.
    Derivado directamente de la expresion de f(z) en 01_generate_sandbox_geometries.py.

    Cada rama tiene su propia derivacion desde f(z) analitico  no hay fallback
    ...
    Tier A:
      rn_ads:         f = 1 - (1+q2)(z/z_h)^d + q2(z/z_h)^{2(d-1)}
      gauss_bonnet:   f = 1 - (z/z_h)^{d+lambda_gb}
      massive_gravity: f = 1 - (1-mg2z_h2)(z/z_h)^d - mg2z2  (mg2=m_g2mg_c1)
      linear_axion:   f = 1 - (1+a2z_h2/d)(z/z_h)^d + a2z2/d  (a2=alpha2)
      charged_hvlif:  f = 1 - (1+q2)(z/z_h)^{eff_d} + q2(z/z_h)^{2(eff_d-1)}

    Tier A ext (2026-04):
      gubser_rocha:   f = [1 - (z/z_h)^d] / (1 + mu_GR  z/z_h)
      soft_wall:      f = 1 - (z/z_h)^d   (toda la firma fisica en A(z))
    """
    #  Tier Canonical 
    if family == "ads":
        exponent = float(d)
    ...
    #  Tier A 
        ...
        # f'(z_h) = [-(1+q2)d + q22(d-1)] / z_h = (-d + q2(d-2)) / z_h
        q = charge_Q
        ...
        # f(z) = 1 - (1-mg2z_h2)(z/z_h)^d - mg2z2,  mg2 = m_g2mg_c1
        # f'(z) = -d(1-mg2z_h2)z^{d-1}/z_h^d - 2mg2z
        # f'(z_h) = [-d(1-mg2z_h2) - 2mg2z_h2] / z_h
        #          = [-d + mg2z_h2(d-2)] / z_h
        mg2 = m_g * m_g * mg_c1
        ...
        # f(z) = 1 - (1+a2z_h2/d)(z/z_h)^d + a2z2/d,  a2 = alpha_axion2
        # f'(z_h) = [-d - a2z_h2 + 2a2z_h2/d] / z_h
        #          = [-d + a2z_h2(2/d - 1)] / z_h
        #          = [-d - a2z_h2(d-2)/d] / z_h
    ...
    #  Tier A ext (2026-04) 
        ...
        # f(z) = 1 - (z/z_h)^d identico a AdS en f; la firma vive en A(z).
        return float(d) / z_h

    else:
        # Fallback generico solo para familias no registradas (e.g. test nuevas)
        return 4.0 / z_h
...
def fp_horizon_effective(
    ...
    """
    Denominador correcto para el exponente  = -i / fp_eff en la BC entrante.

    Del analisis de la ecuacion near-horizon en gauge domain-wall
        ds2 = e^{2A}(-f dt2 + dx2) + dz2/f
    se obtiene que la exponente del modo entrante es:
         = -i / (e^{A(z_h)}  |f'(z_h)|)
    donde el factor e^{A(z_h)} proviene de la ecuacion de Euler-Cauchy
    ...
    # Evalua A ligeramente antes del horizonte para evitar f=0
...
def make_ode(A_spline: CubicSpline, f_spline: CubicSpline,
    ...
    Retorna rhs(z, u) para el sistema:
        u = [, ']
        d/dz  = '
        d'/dz = -[(d+1)A'(z) + f'(z)/f(z)] ' - [2 e^{-2A(z)} / f(z)2] 
    ...
    def rhs(z: float, u: np.ndarray) -> np.ndarray:
        ...
        # Clip para evitar division por cero lejos del horizonte
        fz = max(fz, 1e-14)
...
# 
# Condicion inicial en el horizonte (BC entrante)
# 

def horizon_ic(omega: complex, fp_h: float, eps: float) -> Tuple[complex, complex]:
    """
    Condicion inicial para  a z_start = z_h - eps.

    Solucion entrante:  ~ (z_h-z)^,  = -i/|f'_h|
    Normalizado: (z_start) = 1
    d/dz(z_start) = - / eps
    """
    alpha = -1j * omega / fp_h
    phi0  = 1.0 + 0j
...
# 
# Funcion de shooting: integra y devuelve (z_min)
# 
...
def shoot(omega: complex,
    ...
    Retorna (z_min)  debe ser ~ 0 en un QNM.
    """
...
# 
# Busqueda de un QNM a partir de un guess inicial
# 
...
def find_qnm(omega_guess: complex,
    ...
    """
    Usa fsolve para encontrar el QNM mas cercano a omega_guess.
    Retorna  complejo si converge, None si no.
    """
    ...
    # QNMs tienen Im() < 0 (modos que decaen)
...
def initial_guesses(T: float, family: str, z_dyn: float, theta: float,
    ...
    Los QNMs del gauge domain-wall estan en torno a:
        _R ~ (2n+3)  4T  (escala empirica)
           ~ (0.5..1)  4T
    Por eso usamos 4T = d/z_h como escala base (no 2T).
    """
    scale = 4.0 * np.pi * T   # escala base = 4T
    if scale < 1e-8:
        scale = 1.0

    guesses: List[complex] = []

    # Empiricamente, en gauge domain-wall el modo n-esimo esta en torno a:
    #   _R  (3 + 4n)  4T,    (0.5..1)  4T
    # para AdS. Otras familias tendran escala diferente pero similar estructura.
    base_re = 3.0 + 4.0 * n_mode   # en unidades de scale = 4T
...
def process_geometry(h5path: Path, n_modes: int,
    ...
    """
    Carga la geometria, corre el shooting y retorna un dict con los QNMs.
    """
    with h5py.File(h5path, "r") as h:
        ...
        # Tier A: leer attrs canonicos extra (con defaults para H5 legacy)
    ...
    # Asegurar que f > 0 en z_phys (puede haber pequenos negativos por el clip)
    ...
    #  Denominador correcto para  = -i/fp_h 
    ...
    for n in range(n_modes):
        ...
        if found is None:
            print(f"  [WARN] {name}: no convergio modo n={n}")
...
def print_family_summary(results: List[dict]) -> None:
    ...
    print("\n" + "=" * 62)
    print(f"{'Familia':<16} {'N':>3}  {'f1/f0':>10}  {'1/0':>10}  {'Q0':>8}")
    print("=" * 62)
...
def parse_args():
    ...
    p.add_argument("--n-modes", type=int, default=3,
                   help="Numero de modos QNM a buscar por geometria (default: %(default)s)")
    p.add_argument("--eps-horizon", type=float, default=1e-3,
                   help=" como fraccion de z_h para el punto de inicio cerca del horizonte")
...
def main():
    ...
    print(f"Procesando {len(h5_files)} geometrias  "
    ...
    for i, h5p in enumerate(h5_files):
        ...
        # Formatear salida en linea
        modes_str = "  ".join(
            f"{n}=({r['qnms_re'][n]:.2f}{r['qnms_im'][n]:+.2f}i)"
            if (r["qnms_re"][n] is not None and not np.isnan(r["qnms_re"][n]))
            else f"{n}=NaN"
            for n in range(len(r["qnms_re"]))
        )
        ratio_str = (f"  f1/f0={r['freq_ratio']:.3f}  1/0={r['damp_ratio']:.3f}"
