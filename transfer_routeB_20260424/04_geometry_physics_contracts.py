...
#           * Umbrales de R2, estabilidad en rollouts, etc.
#       - Flags de honestidad (uso indebido de bulk, mezcla incorrecta de d, ...).
#
# TIPOS DE CONTRATO (EJEMPLOS)
#   - R2 minimo en test, estabilidad en evoluciones numericas.
...
#     En este modo, metricas R2 se marcan como null y generic contracts se evaluan
...
#   correlator, not an analytic CFT solution.  Power-law fit quality is typically 0.150.46,
#   well below the r2>0.5 threshold.  The absence of a clean power-law is a property of the
...
class BoundaryUnitarityContract:
    ...
    Cota de unitariedad para escalares:  >= (d-2)/2
    Si algun operador viola esto, el sistema NO puede ser una CFT unitaria
    y por tanto NO puede tener dual holografico valido.
    
    Referencia: AGMOO review, Chunk 04, p.32
    """
    all_operators_unitary: bool  # Todos cumplen  >= (d-2)/2
    n_operators: int
    n_violations: int
    min_delta: float
...
class CorrelatorStructureContract:
    ...
    Una CFT tiene correladores con power-law decay: G(x) ~ x^{-2}
    Ruido blanco tiene G(x) ~ (x) (sin estructura espacial)
    
    Detectamos esto verificando:
    1. Correlador no es constante (tiene variacion espacial)
    2. Tiene decaimiento monotono (no oscila aleatoriamente)
    3. En escala log-log, tiene pendiente negativa (power-law)
    """
    has_spatial_structure: bool  # No es constante/ruido
    is_monotonic_decay: bool  # Decae monotonamente
    has_power_law: bool  # Pendiente log-log negativa
    log_slope: float  # Pendiente en log-log (deberia ser ~ -2)
    correlation_quality: float  # R2 del ajuste log-log
    skipped: bool = False
    contract_mode: str = CORRELATOR_CONTRACT_STANDARD  # versionado explicito
...
class AdSEinsteinContract:
    """Verifica ecuaciones de Einstein en vacio con  (SOLO para AdS)."""
    R_is_constant: bool
    R_matches_ads: bool
...
def verify_boundary_unitarity(
    ...
    Cota de unitariedad para escalares (spin=0):  >= (d-2)/2
    Cota para spin s:  >= s + d - 2 (para s >= 1)
...
def verify_correlator_structure(
    ...
    try:
        ...
        if np.sum(mask) > 5:
            ...
            # coeffs[0] es la pendiente (deberia ser negativa para CFT)
            coeffs = np.polyfit(log_x, log_G, 1)
            log_slope = coeffs[0]
            
            # Calcular R2 del ajuste
...
def process_geometry(
    ...
    Soporta dos modos:
    - sandbox: con bulk_truth y *_truth (metricas R2 calculadas)
    - inference: without truth (metricas R2 = None, generic contracts evaluados)
    """
    errors = []
    warnings = []
    ...
    if mode == "sandbox" and "A_truth" in data:
        ...
        family_match = None
        if mode == "inference":
            warnings.append("inference mode: metricas R2 no disponibles")
...
def main():
    ...
    if HAS_STAGE_UTILS:
        ...
        try:
            ...
            # Fallback legacy: permitir auditorias locales con rutas explicitas aunque no
    ...
    try:
        ...
        # Permite que el smoke documentado funcione aunque falte ese modulo auxiliar.
        ...
        if contract.errors:
            ...
            if contract.A_r2 is not None:
                print(f"   R2: A={contract.A_r2:.3f}, f={contract.f_r2:.3f}")
            else:
                print(f"   R2: (no disponible en inference)")
        ...
        if n_total == 0:
            print(" NO HAY GEOMETRIAS PARA EVALUAR")
        elif phase_passed:
